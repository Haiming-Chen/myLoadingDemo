...
/**
 * Created by admin-chen on 2017/4/6.
 */
...
public class Loading extends View {
    ...
    // 叶子飘动一个周期所花的时间
    private static final long LEAF_FLOAT_TIME = 1200;
    // 叶子旋转一周需要的时间
    private static final long LEAF_ROTATE_TIME = 800;
    //叶子的最大数值
    ...
    private void initPaint() {
        ...
        mPaint1.setDither(true);//设置防抖动
        mPaint1.setFilterBitmap(true);//对位图进行滤波处理

        mPaint2 = new Paint();
        mPaint2.setColor(WHITE_COLOR);
        mPaint2.setAntiAlias(true);

        mPaint3 = new Paint();
        mPaint3.setColor(ORANGE_COLOR);
        mPaint3.setAntiAlias(true);
    }

    private void initBitmap( Context context) {
        mLeafBitmap = ((BitmapDrawable) ContextCompat.getDrawable(context,R.drawable.leaf)).getBitmap();
        mLeafWidth = mLeafBitmap.getWidth();
        mLeafHeight = mLeafBitmap.getHeight();

        mOuterBitmap = ((BitmapDrawable) ContextCompat.getDrawable(context,R.drawable.leaf_kuang)).getBitmap();
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        drawRectandLeafs(canvas);
        canvas.drawBitmap(mOuterBitmap, mOuterSrcRect, mOuterDestRect, mPaint1);
        postInvalidate();
    }
    ...
    private void drawRectandLeafs(Canvas canvas) {
        ...
        if (mCurrentProgressPosition < mArcRadius) {
            // 1.绘制白色ARC
            canvas.drawArc(mArcRectF, 90, 180, false, mPaint2);

            // 2.绘制白色矩形
            mWhiteRectF.left = mArcRightLocation;
            canvas.drawRect(mWhiteRectF, mPaint2);
            ...
            canvas.drawArc(mArcRectF, startAngle, sweepAngle, false, mPaint3);
        } else {
            // 1.绘制white RECT
            ...
            drawLeafs(canvas);
            // 2.绘制Orange ARC
            canvas.drawArc(mArcRectF, 90, 180, false, mPaint3);
            // 3.绘制orange RECT
            mOrangeRectF.left = mArcRightLocation;
            mOrangeRectF.right = mCurrentProgressPosition;
            canvas.drawRect(mOrangeRectF, mPaint3);
        }
    }
    ...
    private void drawLeafs(Canvas canvas){
        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (currentTime > leaf.startTime && leaf.startTime != 0) {
                ...
                canvas.drawBitmap(mLeafBitmap, matrix, mPaint1);
                canvas.restore();
            } else {
                continue;
            }
